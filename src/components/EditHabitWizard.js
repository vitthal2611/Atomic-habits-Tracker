...
const EditHabitWizard = ({ habit, onComplete, onCancel, habits, loading }) => {
  ...
  const usedHabits = new Set(habits?.filter(h => h.id !== habit.id).map(h => h.afterHabit).filter(Boolean) || []);
  ...
  const { values: formData, errors, touched, handleChange, handleBlur, validateAll, setValues } = useFormValidation({
    ...
    visualCue: habit.visualCue || '',
    afterHabit: habit.afterHabit || '',
    ...
    <div className="form-wizard-overlay">
      <div className="form-wizard">
        ...
        <div className="wizard-content">
          ...
          {step === 5 && (
            <div className="wizard-step fade-in">
              <h2 className="step-title">Review your changes</h2>
...
export default EditHabitWizard;